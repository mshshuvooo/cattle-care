...
**Search component usage rules:**
- Import from `@/Components/Search/Search.vue`
- Props: `route` (required, named route string), `routeParam` (object, for route params), `modelValue` (current search string), `param` (query param name, default `'search'`)
- Debounces navigation by 300ms; cleans up on unmount
- Usage: `<Search route="users.index" :model-value="search" />`
