...
const form = useForm({
    bull_id:          props.aiSire?.data?.bull_id ?? '',
    name:             props.aiSire?.data?.name ?? '',
    breed:            props.aiSire?.data?.breed ?? '',
    breed_percentage: props.aiSire?.data?.breed_percentage ?? '',
    owner:            props.aiSire?.data?.owner ?? '',
})

const submit = () => {
    form.put(route('ai-sires.update', props.aiSire?.data?.id))
}
</script>

<template>
    <AppLayout title="Edit AI Sire">

        <FormSection @submitted="submit">
            <template #title>
                <h3 class="text-sm font-semibold text-gray-800">AI Sire Details</h3>
                <p class="text-xs text-gray-400 mt-0.5">Update bull identification and semen source information</p>
            </template>
